  ...
  return (
    <div className="animate-fade-in">
      <div className="mb-6">
        ...
        <p className="text-[#aaa] text-sm mt-0.5">{starred.length} starred files</p>
      </div>

      {starred.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-64 upload-zone">
          <Star className="w-12 h-12 text-yellow-400/30 mb-3" />
          <p className="text-[#888] font-display font-semibold">No starred files</p>
          <p className="text-[#aaa] text-sm mt-1">Star important files for quick access</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          {starred.map(file => (
            <div key={file.id} className="file-card group">
              ...
              <p className="text-xs font-medium text-[#333] truncate mb-1">{file.name}</p>
              <div className="flex justify-between items-center">
                <p className="text-[10px] text-[#aaa]">{formatBytes(file.size)}</p>
                <p className="text-[10px] text-[#aaa]">{formatDate(file.created_at)}</p>
              </div>
