...
export default function FilesClient({ initialFiles, initialFolders, storageUsed = 0 }: Props) {
  ...
  async function openPreview(file: StorageFile) {
    ...
    setPreviewUrl(null);
    const supabase = createClient();
    const { data } = await supabase.storage.from(STORAGE_BUCKET).createSignedUrl(file.storage_path, 300);
    if (data?.signedUrl) setPreviewUrl(data.signedUrl);
    setPreviewLoading(false);
  }

  function closePreview() { setPreviewFile(null); setPreviewUrl(null); }
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    for (const file of acceptedFiles) {
      if (file.size > MAX_FILE_SIZE) { showToast(`${file.name} exceeds 50MB limit`, "error"); continue; }
      ...
      if (uploadError) {
        ...
        showToast(`Failed to upload ${file.name}`, "error"); continue;
      }
      ...
      const { data: newFile, error: dbError } = await supabase.from("files").insert({
        ...
        file_type: fileType, is_starred: false, is_deleted: false, tags: [], download_count: 0,
      }).select().single();
      if (!dbError && newFile) {
  ...
  async function downloadFile(file: StorageFile) {
    ...
    } catch (err: any) { showToast(err?.message ?? "Download failed", "error"); }
  }
  ...
  function renderPreviewBody() {
    if (previewLoading) return <Loader2 className="w-10 h-10 text-gray-400 animate-spin" />;
    if (!previewUrl) return <p className="text-[#aaa] text-sm">Failed to load preview</p>;
        ...
        return (
          <object data={previewUrl} type="application/pdf" className="w-full h-full">
            ...
            <div className="flex flex-col items-center gap-4 p-8 text-center">
              ...
              <p className="text-[#333] font-medium">{previewFile.name}</p>
              <div className="flex gap-3">
                <a href={previewUrl} target="_blank" rel="noopener noreferrer" className="btn-primary text-sm py-2 px-4 flex items-center gap-2">Open in new tab</a>
                <button onClick={() => downloadFile(previewFile)} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-[#f5f5f5] text-[#333] text-sm border border-black/[0.08]"><Download className="w-4 h-4" /> Download</button>
              </div>
            </div>
          </object>
        );
      case "image": return <img src={previewUrl} alt={previewFile?.name} className="max-w-full max-h-full object-contain" />;
      case "video": return <video src={previewUrl} controls className="max-w-full max-h-full" />;
      case "audio":
        return (
      ...
      default:
        return (
          <div className="flex flex-col items-center gap-4 text-center p-8">
            ...
            <p className="text-[#333] font-medium">{previewFile?.name}</p>
            <p className="text-[#aaa] text-sm">Preview not available for this file type</p>
            <button onClick={() => previewFile && downloadFile(previewFile)} className="btn-primary text-sm py-2 px-4 flex items-center gap-2"><Download className="w-4 h-4" /> Download to view</button>
          </div>
        );
    }
  }
  ...
  return (
    <div className="relative">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 style={{ ...SYNE, fontSize: "28px", color: "#0a0a0a" }}>My Files</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-5">
        {[
          ...
          <div key={label} className="bg-white py-3 px-3.5 transition-transform hover:-translate-y-0.5"
            ...
            <div style={{ ...SYNE, fontSize: "20px", color: "#0a0a0a", lineHeight: 1, marginBottom: "6px" }}>{value}</div>
      ...
      <div className="flex gap-1 mb-4">
        {([ { key: "all", label: "All" }, { key: "image", label: "Images" }, { key: "doc", label: "Documents" }, { key: "video", label: "Videos" } ] as { key: TabType; label: string }[]).map(tab => (
          <button key={tab.key} onClick={() => setActiveTab(tab.key)}
            className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${activeTab === tab.key ? "bg-[#0a0a0a] text-white" : "text-[#aaa] hover:text-[#555]"}`}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-3 mb-5">
        {filtered.length > 0 && (
          <button onClick={selectAll} className="flex items-center gap-2 text-xs text-[#aaa] hover:text-[#555] transition-colors">
        ...
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search files..."
            className="w-full px-3 py-2 rounded-xl text-[#333] placeholder-[#aaa] text-sm focus:outline-none transition-all"
            style={{ paddingLeft: "2.5rem", background: "#f5f5f5", border: "1px solid rgba(0,0,0,0.08)" }} />
        </div>
        <div className="flex rounded-lg border border-black/[0.08] overflow-hidden">
          <button onClick={() => setView("grid")} className={`p-2 transition-colors ${view === "grid" ? "bg-[#0a0a0a] text-white" : "text-[#aaa] hover:text-[#555] bg-transparent"}`}><Grid3X3 className="w-4 h-4" /></button>
          <button onClick={() => setView("list")} className={`p-2 transition-colors ${view === "list" ? "bg-[#0a0a0a] text-white" : "text-[#aaa] hover:text-[#555] bg-transparent"}`}><List className="w-4 h-4" /></button>
        </div>
      </div>
      ...
      {uploads.length > 0 && (
        <div className="mb-4 space-y-2">
          {uploads.map((u, i) => (
            <div key={i} className="rounded-xl p-3 flex items-center gap-3" style={{ background: "#fafafa", border: "1px solid rgba(0,0,0,0.07)" }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="progress-bar"><div className="progress-fill" style={{ width: `${u.progress}%`, background: u.status === "error" ? "#ef4444" : undefined }} /></div>
              </div>
      ...
      {/* ── Section title ── */}
      {filtered.length > 0 && (
        <div className="text-[11px] font-semibold text-[#aaa] uppercase tracking-widest mb-3">Recent Files</div>
      )}

      {/* ── Grid view ── */}
      {view === "grid" && filtered.length > 0 && (
        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-2 pb-4">
          {filtered.map(file => {
            ...
            return (
              <div key={file.id} onClick={() => openPreview(file)}
                ...
                style={{ borderRadius: 12, border: "0.5px solid rgba(0,0,0,0.07)", borderLeft: `2px solid ${glowColor}`, boxShadow: `-3px 0 10px ${glowColor}33, 0 4px 14px ${glowColor}12`, padding: "10px" }}>
                <div onClick={e => toggleSelect(file.id, e)}
                  className={`absolute top-2 left-2 z-10 w-5 h-5 rounded border flex items-center justify-center transition-all ${selected.has(file.id) ? "bg-[#0a0a0a] border-[#0a0a0a] opacity-100" : "border-black/20 bg-white/80 opacity-0 group-hover:opacity-100"}`}>
                  {selected.has(file.id) && <span className="text-white text-[10px]">✓</span>}
                </div>
                <div className="flex items-center justify-center h-12 mb-2">
                  <div className="text-3xl">{FILE_ICONS[file.file_type]}</div>
                </div>
                <p className="text-xs font-semibold text-[#333] truncate mb-0.5" title={file.name}>{file.name}</p>
                <p className="text-[9px] text-[#aaa] font-mono">{formatBytes(file.size)}</p>
                <span className={`badge badge-${file.file_type} mt-1.5`}>{file.file_type}</span>
                <div className="flex items-center justify-between mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button type="button" onClick={e => { e.stopPropagation(); toggleStar(file); }}
                    ...
                    <Star className="w-3 h-3" fill={file.is_starred ? "currentColor" : "none"} />
                  </button>
                  <div className="flex gap-0.5">
                    <button type="button" onClick={e => { e.stopPropagation(); downloadFile(file); }} className="p-1 rounded text-[#ccc] hover:text-[#0a0a0a] transition-colors"><Download className="w-3 h-3" /></button>
                    <button type="button" onClick={e => { e.stopPropagation(); shareFile(file); }} className="p-1 rounded text-[#ccc] hover:text-green-600 transition-colors"><Share2 className="w-3 h-3" /></button>
                    <button type="button" onClick={e => { e.stopPropagation(); moveToTrash(file); }} className="p-1 rounded text-[#ccc] hover:text-red-500 transition-colors"><Trash2 className="w-3 h-3" /></button>
                  </div>
                </div>
              </div>
            );
      ...
      {view === "list" && filtered.length > 0 && (
        <div className="rounded-xl overflow-hidden pb-4" style={{ border: "1px solid rgba(0,0,0,0.07)" }}>
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: "1px solid rgba(0,0,0,0.07)", background: "#fafafa" }}>
                <th className="px-4 py-3 w-8">
                  <div onClick={selectAll} className={`w-4 h-4 rounded border flex items-center justify-center cursor-pointer transition-all ${allSelected ? "bg-[#0a0a0a] border-[#0a0a0a]" : "border-black/20"}`}>
                    {allSelected && <span className="text-white text-[10px]">✓</span>}
                    {!allSelected && someSelected && <span className="text-white text-[10px]">–</span>}
                  </div>
                </th>
                {["Name", "Size", "Type", "Date", ""].map(h => (
                  <th key={h} className="text-left text-xs font-semibold px-4 py-3" style={{ color: "#aaa" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map(file => (
                <tr key={file.id} onClick={() => openPreview(file)}
                  className={`group cursor-pointer transition-colors hover:bg-[#fafafa] ${selected.has(file.id) ? "bg-[#f5f5f5]" : ""}`}
                  style={{ borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
                  <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                    <div onClick={e => toggleSelect(file.id, e)} className={`w-4 h-4 rounded border flex items-center justify-center cursor-pointer transition-all ${selected.has(file.id) ? "bg-[#0a0a0a] border-[#0a0a0a]" : "border-black/20"}`}>
                      {selected.has(file.id) && <span className="text-white text-[10px]">✓</span>}
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-semibold text-[#333] truncate max-w-[200px]">{file.name}</span>
                      {file.is_starred && <Star className="w-3 h-3 text-yellow-500 fill-current flex-shrink-0" />}
                    </div>
                  </td>
                  <td className="px-4 py-3 text-xs text-[#aaa] font-mono">{formatBytes(file.size)}</td>
                  <td className="px-4 py-3"><span className={`badge badge-${file.file_type}`}>{file.file_type}</span></td>
                  <td className="px-4 py-3 text-xs text-[#aaa]">{formatDate(file.created_at)}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity justify-end">
                      <button type="button" onClick={e => { e.stopPropagation(); downloadFile(file); }} className="p-1.5 rounded-lg text-[#ccc] hover:text-[#0a0a0a] hover:bg-black/[0.05] transition-colors"><Download className="w-3.5 h-3.5" /></button>
                      <button type="button" onClick={e => { e.stopPropagation(); shareFile(file); }} className="p-1.5 rounded-lg text-[#ccc] hover:text-green-600 hover:bg-green-50 transition-colors"><Share2 className="w-3.5 h-3.5" /></button>
                      <button type="button" onClick={e => { e.stopPropagation(); toggleStar(file); }} className={`p-1.5 rounded-lg transition-colors ${file.is_starred ? "text-yellow-500" : "text-[#ccc] hover:text-yellow-500 hover:bg-yellow-50"}`}><Star className="w-3.5 h-3.5" fill={file.is_starred ? "currentColor" : "none"} /></button>
                      <button type="button" onClick={e => { e.stopPropagation(); moveToTrash(file); }} className="p-1.5 rounded-lg text-[#ccc] hover:text-red-500 hover:bg-red-50 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {filtered.length === 0 && uploads.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-lg font-semibold text-[#aaa] mb-1" style={SYNE}>{search ? "No files found" : "No files yet"}</p>
          <p className="text-[#bbb] text-sm max-w-xs">{search ? "Try a different search term" : "Upload files to get started"}</p>
        </div>
      )}
      ...
      {previewFile && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center" onClick={closePreview}>
          ...
          <div className="relative z-10 flex flex-col w-[90vw] h-[90vh] max-w-5xl rounded-2xl overflow-hidden shadow-2xl"
            ...
            <div className="flex items-center justify-between px-5 py-3 flex-shrink-0" style={{ borderBottom: "1px solid rgba(0,0,0,0.07)" }}>
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-[#333] truncate">{previewFile.name}</p>
                  <p className="text-xs text-[#aaa] font-mono">{formatBytes(previewFile.size)}</p>
                </div>
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <button onClick={() => downloadFile(previewFile)}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors"
                  style={{ background: "#f5f5f5", color: "#333", border: "1px solid rgba(0,0,0,0.08)" }}>
                  <Download className="w-3.5 h-3.5" /> Download
                </button>
                <button onClick={closePreview} className="p-1.5 rounded-lg text-[#aaa] hover:text-[#333] hover:bg-[#f5f5f5] transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
            </div>
            <div className="flex-1 overflow-hidden flex items-center justify-center" style={{ background: "#fafafa" }}>
              {renderPreviewBody()}
            </div>
          </div>
        </div>
