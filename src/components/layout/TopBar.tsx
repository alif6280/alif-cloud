...
export default function TopBar({ user, profile }: TopBarProps) {
  ...
  return (
    <header className="flex items-center justify-between px-4 sm:px-6 py-3.5 border-b border-white/[0.04] bg-[#08080f]/80 backdrop-blur-xl flex-shrink-0">
      {/* Left: search (with left padding on mobile for the hamburger button) */}
      <div className="relative w-full max-w-xs pl-12 lg:pl-0">
        <Search className="absolute left-12 lg:left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 pointer-events-none z-10" />
        <input
          type="text"
          value={search}
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0 ml-3">
        ...
        <div className="flex items-center gap-2 pl-2 sm:pl-3 border-l border-white/[0.06]">
          <div className="w-8 h-8 rounded-xl bg-brand-600/30 border border-brand-500/30 flex items-center justify-center text-brand-300 text-xs font-bold font-display flex-shrink-0">
            {initials}
          </div>
          <div className="hidden sm:block">
            <div className="text-sm font-medium text-white leading-none truncate max-w-[100px]">{name}</div>
